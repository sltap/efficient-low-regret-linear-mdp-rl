...
def eval_policy(policy, eval_env, eval_episodes=10):
	...
	for _ in range(eval_episodes):
		ret, done = eval_env.reset(), False
		state = ret[0]
		while not done:
			action = int(np.rint(policy.select_action(np.array(state)))[0]) # for discrete action spaces
			state, reward, terminated, truncated, _ = eval_env.step(action)
			done = terminated or truncated # Maintain compatibility with old version of gym
			avg_reward += reward
